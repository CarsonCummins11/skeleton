locals {
  domain_parts_for_root = split(".", var.domain_name)
  domain_parts_len = length(local.domain_parts_for_root)

  # Extract the root domain from var.domain_name
  root_domain = join(".", slice(local.domain_parts_for_root, local.domain_parts_len - 2, local.domain_parts_len))
}
...
resource "aws_acm_certificate_validation" "service_cert_validation" {
  ...
  validation_record_fqdns = [
    for record in aws_route53_record.service_cert_validation_record : record.fqdn
  ]

  depends_on = [
    aws_route53_record.service_cert_validation_record,
  ]
}
